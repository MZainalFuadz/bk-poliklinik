...
// Fungsi untuk memuat jadwal berdasarkan ID poli dan status
function loadJadwal($pdo, $poli_id)
{
  $query = $pdo->prepare("SELECT * FROM jadwal_periksa WHERE id_poli = :poli_id AND aktif = 'Y'");
  $query->execute(['poli_id' => $poli_id]);
  $options = "";
  while ($row = $query->fetch()) {
    $options .= "<option value='" . $row['id'] . "'>" . $row['hari'] . " " . $row['jam_mulai'] . " - " . $row['jam_selesai'] . "</option>";
  }
  return $options;
}
?>
...
<body class="hold-transition sidebar-mini layout-fixed">
  ...
  <?php
  if ($_SERVER['REQUEST_METHOD'] === 'GET' && isset($_GET['poli_id'])) {
    $poli_id = $_GET['poli_id'];
    echo loadJadwal($pdo, $poli_id);
    exit();
  }
  ?>
</body>
